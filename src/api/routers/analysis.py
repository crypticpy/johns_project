...
def _build_comparison_section(db: Session, compare_dataset_id: int) -> Tuple[str, Dict[str, Any]]:
    ...
    if ds2 is None:
        raise HTTPException(status_code=404, detail=f"Comparison dataset {compare_dataset_id} not found")
    ...
    if complexity2:
        lines.append("Complexity: " + ", ".join([f"{k}({v})" for k, v in sorted(complexity2.items())]))
...
async def run_analysis(
    ...
    if analyzer_backend is not None:
        if str(analyzer_backend).strip().lower() not in ("openai", "offline"):
            raise HTTPException(status_code=400, detail="analyzer_backend must be 'openai' or 'offline'")
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Invalid sampling configuration")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to persist analysis: {e}")
