...
from ai.llm.tools.registry import ToolRegistry, ToolContext


@pytest.mark.unit
def test_tool_call_count_increments_when_metrics_available(registry: ToolRegistry, ctx_viewer: ToolContext):
    """
    ...
    initial = float(getattr(labels, "_value").get())  # type: ignore[attr-defined]
    ...
    final = float(getattr(labels, "_value").get())  # type: ignore[attr-defined]
    assert final - initial == 3, f"Expected counter to increase by 3, initial={initial}, final={final}"