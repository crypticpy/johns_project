...
class DatasetManager:
    ...
    def get_comparison_stats(self) -> pd.DataFrame:
        ...
        for name, df in self.datasets.items():
            stat_entry = {
                ...
                "Departments": df["Department"].nunique() if "Department" in df.columns else 0,
                "Assignment Groups": (
                    df["Assignment Group"].nunique() if "Assignment Group" in df.columns else 0
                ),
            }
            ...
            if "Reassignment group count tracking_index" in df.columns:
                stat_entry["Avg Reassignments"] = df[
    ...
    def _quality_score(self, quality_series: pd.Series) -> float:
        mapping = {"excellent": 5, "good": 4, "fair": 3, "poor": 2, "very poor": 1}
...
class ChartGenerator:
    @staticmethod
    def create_quality_distribution(df: pd.DataFrame) -> go.Figure | None:
    ...
    @staticmethod
    def create_complexity_distribution(df: pd.DataFrame) -> go.Figure | None:
        ...
        fig = px.bar(
            ...
            x="Complexity",
            y="Count",
            title="Resolution Complexity Distribution",
            color="Count",
    ...
    @staticmethod
    def create_department_volume(df: pd.DataFrame, top_n: int = 10) -> go.Figure | None:
        ...
        fig = px.bar(
            ...
            y="Department",
            orientation="h",
            title=f"Top {top_n} Departments by Ticket Volume",
            color="Count",
    ...
    @staticmethod
    def create_reassignment_analysis(df: pd.DataFrame) -> go.Figure | None:
        ...
        fig = go.Figure(
            data=[
                go.Bar(
                    x=reassignment_counts["Reassignments"],
                    y=reassignment_counts["Count"],
                    marker_color="indianred",
                )
            ]
        )
        fig.update_layout(
            title="Reassignment Distribution", xaxis_title="Reassignments", yaxis_title="Count"
        )
        return fig

    @staticmethod
    def create_product_distribution(df: pd.DataFrame, top_n: int = 15) -> go.Figure | None:
        ...
        fig = px.bar(
            ...
            y="Product/System",
            orientation="h",
            title=f"Top {top_n} Products/Systems by Ticket Volume",
            color="Count",
...
class OnboardingAnalyzer:
    def __init__(self, azure_endpoint: str, api_key: str, deployment_name: str) -> None:
        self.client = AzureOpenAI(
            azure_endpoint=azure_endpoint, api_key=api_key, api_version="2024-02-15-preview"
        )
        self.deployment_name = deployment_name

    def prepare_ticket_context(
        self, df: pd.DataFrame, max_tickets: int = 50, dataset_name: str = "Dataset"
    ) -> tuple[str, dict]:
        relevant_cols = [
            "Description",
            "Assignment Group",
        ...
        if "Reassignment group count tracking_index" in df.columns:
            reassignment_counts = (
                df["Reassignment group count tracking_index"].value_counts().to_dict()
            )
    ...
    def analyze_onboarding_opportunities(
        self, ticket_context: str, user_question: str, comparison_mode: bool = False
    ) -> str:
        system_prompt = (
...
def main():
    # Page configuration
    st.set_page_config(page_title="Service Desk Onboarding Analyzer", page_icon="🎫", layout="wide")
    ...
    **Note:** Upload multiple datasets to compare different snapshots (e.g., different departments, time periods you've exported separately, or before/after intervention scenarios).
    ...
    azure_endpoint = st.sidebar.text_input(
        "Azure OpenAI Endpoint", placeholder="https://your-resource.openai.azure.com/"
    )
    azure_api_key = st.sidebar.text_input("API Key", type="password")
    azure_deployment = st.sidebar.text_input("Deployment Name", placeholder="gpt-4")
    ...
    with tabs[0]:
        ...
        with col1:
            uploaded_files = st.file_uploader(
                "Upload Excel datasets (.xlsx)", type=["xlsx"], accept_multiple_files=True
            )
            if uploaded_files:
                for uploaded_file in uploaded_files:
                    try:
                        ...
                        st.success(
                            f"Dataset '{dataset_name}' loaded successfully with {len(df)} tickets."
                        )
                    except Exception as e:
    ...
    with tabs[1]:
        ...
        if analysis_mode == "Single Dataset":
            selected_dataset_name = st.selectbox(
                "Select Dataset", datasets, key="analysis_single_dataset_select"
            )
            selected_datasets = [selected_dataset_name] if selected_dataset_name else []
        else:
            selected_datasets = st.multiselect(
                "Select Datasets", datasets, default=datasets, key="analysis_multi_dataset_select"
            )
        ...
        question_mode = st.radio(
            "Question Mode", options=["Single Question", "Batch Questions"], horizontal=True
        )
        user_question = ""
        ...
        if question_mode == "Single Question":
            ...
            selected_question_option = st.selectbox(
                ...
                example_questions,
                key="analysis_single_question_select",
            )
            if selected_question_option == "Custom question...":
                user_question = st.text_area(
                    "Your question:", height=100, key="custom_question_textarea"
                )
            else:
                user_question = st.text_area(
                    "Your question:",
                    value=selected_question_option,
                    height=100,
                    key="single_question_textarea",
                )
            ...
            if st.button("➕ Add to Batch"):
                if new_batch_question.strip():
                    ...
                    st.success(
                        f"Added to batch. Total questions: {len(st.session_state['batch_questions'])}"
                    )
        ...
        analyze_label = (
            "🚀 Analyze"
            if question_mode == "Single Question"
            else f"🚀 Run Batch Analysis ({len(st.session_state['batch_questions'])} questions)"
        )
        ...
        if st.button(analyze_label, type="primary", disabled=analyze_disabled):
            ...
            if question_mode == "Single Question":
                ...
                if len(filtered_datasets) == 1:
                    ...
                    save_analysis(
                        ds_name,
                        st.session_state["dataset_manager"].metadata[ds_name]["hash"],
                    ...
                    st.download_button(
                        "💾 Download Analysis", data=analysis, file_name="analysis.md"
                    )
                else:
                    # Comparison mode
                    combined_contexts = []
                    ...
                    analysis = analyzer.analyze_onboarding_opportunities(
                        combined_context_str, user_question, comparison_mode=True
                    )
                    ...
                    st.markdown(analysis)
                    st.download_button(
                        "💾 Download Analysis", data=analysis, file_name="comparison_analysis.md"
                    )
            else:
                # Batch mode
    ...
    with tabs[2]:
        ...
        if not datasets:
            ...
            selected_dataset_name = st.selectbox(
                "Select Dataset for Visualization", datasets, key="visualization_dataset_select"
            )
            ...
            if selected_df is not None:
                ...
                if len(datasets) > 1:
                    ...
                    comparison_metric = st.selectbox(
                        ...
                        ["quality", "complexity"],
                        key="visualization_comparison_metric_select",
                    )
                    comparison_datasets = {
                        name: st.session_state["dataset_manager"].get_dataset(name)
                        for name in datasets
                    }
                    fig_comparison = None
                        ...
                        for name, df in comparison_datasets.items():
                            if "ticket_quality" in df.columns:
                                ...
                                fig_comparison.add_trace(
                                    go.Bar(name=name, x=counts.index, y=counts.values)
                                )
    ...
    with tabs[4]:
        ...
        selected_dataset_name = st.selectbox(
            "Select Dataset for Report", datasets, key="reports_dataset_select"
        )
        selected_df = st.session_state["dataset_manager"].get_dataset(selected_dataset_name)
            ...
            if st.button("📄 Generate Report", type="primary"):
                ...
                with st.spinner("Generating comprehensive report..."):
                    ...
                    for question in report_questions:
                        context, metrics = analyzer.prepare_ticket_context(
                            selected_df, max_tickets, selected_dataset_name
                        )
                        analysis = analyzer.analyze_onboarding_opportunities(context, question)
                    ...
                    report_md = generate_report(
                        selected_dataset_name, selected_df, analyses, charts
                    )
...
def generate_report(dataset_name: str, df: pd.DataFrame, analyses: list, charts: list) -> str:
    md = "# Automated Onboarding Report\n\n"
